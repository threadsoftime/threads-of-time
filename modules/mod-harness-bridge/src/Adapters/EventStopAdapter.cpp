...
//
// EventStopAdapter — deactivate a game event via GameEventMgr::StopEvent.
//
...
//
// On success, calls sGameEventMgr->StopEvent(event_id, false).
//   overwrite=false: does NOT rebase the End timestamp.
//
// Result: { "stopped": true, "event_id": <u16>, "is_active_now": <bool> }
//
// NOTE: this adapter is main-thread-only (runs inside OnTickDrain, called from
//       WorldScript::OnUpdate).  Thread safety: same rules as EventStartAdapter.
...
{
    DispatchResult EventStop(nlohmann::json const& args)
    {
        DispatchResult res;
        ...
        if (!args.contains("event_id") || !args["event_id"].is_number_integer())
        {
            ...
            res.error_message = "event.stop: event_id (integer) required";
            return res;
        }
        ...
        {
            res.outcome = DispatchResult::Outcome::BadArgs;
            res.error_message = "event.stop: event_id out of range [1, " +
        ...
        {
            res.outcome = DispatchResult::Outcome::BadArgs;
            res.error_message = "event.stop: event_id " + std::to_string(event_id) +
                                " is not a valid event (Length==0 and State==NORMAL)";
            return res;
        }

        // ---- execute ----
        // overwrite=false: preserve the original End date; the slice drives transitions
        // without rebasing the date metadata.
        sGameEventMgr->StopEvent(event_id, false);

...
#
# Lineage: adapted from the original AC fork's image/build.sh. The key
...
# patch-overlay rsync. The script rsyncs the working tree onto the build
# host, runs the AC Dockerfile to build the worldserver + db-import images,
# and tags them.
#
# Discipline preserved from the source script:
#   - rsync to the build host (build runs there, not on the laptop)
#   - -j4 cap (never raise even when "just this once")
#   - worldserver binary mtime cross-check after build (BUILD OK from the
#     script has been historically unreliable; cross-check is mandatory
...
#   BUILD_TOT_HOME   — runtime data root on the build host (default: /opt/tot)
#
# Run from laptop. Requires `ssh "$BUILD_HOST"` key auth.
set -euo pipefail
...
TOT_HOME="${BUILD_TOT_HOME:-/opt/tot}"
...
step "Preparing build directory on the build host"
ssh "$BUILD_HOST" "rm -rf ${BUILD_DIR} && mkdir -p ${BUILD_DIR}"

step "Rsyncing repo working tree to the build host (${BUILD_DIR})"
# Exclude build artifacts, virtualenvs, and the deploy/release run-state
# directories (the build host doesn't need them to compile the worldserver).
...
rsync -a --delete \
    ...
    --exclude='/var/' \
    --exclude='/env/' \
    "$REPO_ROOT/" "$BUILD_HOST:${BUILD_DIR}/"
...
# path) have it baked in by ToT's CI. Here on the build host we rsync from the
# existing install at ${TOT_HOME}/source/modules/mod-playerbots/.
# Falls back to git clone if the local install isn't present.
ssh "$BUILD_HOST" "
    set -e
    if [ -d ${TOT_HOME}/source/modules/mod-playerbots ]; then
        echo 'Using existing mod-playerbots install at ${TOT_HOME}/source/modules/mod-playerbots'
        rsync -a --delete --exclude='.git' \\
            ${TOT_HOME}/source/modules/mod-playerbots/ \\
...
BUILD_AGE=$(ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman run --rm \
...
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman tag wow-server:${TAG} wow-server:current"
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman tag wow-db-import:${TAG} wow-db-import:current"
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman images wow-server"
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman images wow-db-import"

step "Provisioning module .conf files on host bind-mount"
# The Quadlet bind-mounts ${TOT_HOME}/etc → /azerothcore/env/dist/etc,
...
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S bash -c '
    TMP=\$(mktemp -d)
    ...
    mkdir -p ${TOT_HOME}/etc/modules
    for dist in \${TMP}/extract/*.conf.dist; do
        [[ -f \"\$dist\" ]] || continue
        name=\$(basename \"\$dist\")
        cp -f \"\$dist\" ${TOT_HOME}/etc/modules/\"\$name\"
        target=${TOT_HOME}/etc/modules/\"\${name%.dist}\"
        if [[ ! -f \"\$target\" ]]; then
            cp \"\$dist\" \"\$target\"
            echo \"  seeded \${name%.dist} from \$name\"
        fi
    done
    chmod 644 ${TOT_HOME}/etc/modules/*.conf* 2>/dev/null || true
    rm -rf \${TMP}
'"

step "Syncing modules + src trees to ${TOT_HOME}/source/ (runtime + incremental builds)"
...
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S bash -c '
    mkdir -p ${TOT_HOME}/source &&
    rsync -a --delete ${BUILD_DIR}/modules/ ${TOT_HOME}/source/modules/ &&
    rsync -a --delete ${BUILD_DIR}/src/ ${TOT_HOME}/source/src/ &&
    chown -R 1000:1000 ${TOT_HOME}/source/modules ${TOT_HOME}/source/src
'"

step "Smoke test: worldserver binary present"
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman run --rm --entrypoint /bin/ls localhost/wow-server:current /azerothcore/env/dist/bin/"

step "Smoke test: modules linked in"
ssh "$BUILD_HOST" "echo '$SUDO_PW' | sudo -S podman run --rm --entrypoint /bin/sh localhost/wow-server:current -c 'strings /azerothcore/env/dist/bin/worldserver 2>/dev/null | grep -i -E \"playerbot|progression|eluna|autobalance|bracket|warforged|harness\" | sort -u | head -20'"

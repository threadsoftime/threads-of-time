...
//!
//! Port of `harness_daemon/mcp_auth.py` + the spike's `BearerAuthLayer` pattern.
//!
//! On each request:
...
//! - If found → insert the full `config::TokenRecord` (cloned) into
//!   `req.extensions_mut()` and call the inner service.
//!
//! The injected `TokenRecord` is then read inside tool handlers via
//! `Extension<http::request::Parts>` → `parts.extensions.get::<TokenRecord>()`.
...
///
/// We concrete-type the Response to `BoxBody<Bytes, Infallible>` because that
/// is exactly what `StreamableHttpService` returns — matching the spike's body
/// type avoids a generic Body bound on `S` that would infect the whole tower.
#[derive(Clone)]
    ...
    S::Future: Send + 'static,
{
    ...
    fn call(&mut self, mut req: Request<Body>) -> Self::Future {
        ...
        // ── Lookup in TokenStore ───────────────────────────────────────────
        let maybe_record: Option<TokenRecord> = raw_token
            .as_deref()
            .and_then(|tok| self.token_store.find(tok))
            .cloned();
...
mod tests {
    ...
    fn make_token_store() -> Arc<TokenStore> {
        Arc::new(TokenStore::new(vec![TokenRecord {
    ...
    // ── 5. Valid bearer — TokenRecord identity field verified ─────────────────

    #[tokio::test]
    async fn valid_bearer_token_record_fields_correct() {
        // Build a store with two tokens; use the second one.
        let store = Arc::new(TokenStore::new(vec![
            TokenRecord {

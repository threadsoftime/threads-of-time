//! MCP adapter — exposes the 54-tool V1 surface over the MCP StreamableHTTP
...
) -> BearerAuthService<TokenStore, StreamableHttpService<HarnessMcp, LocalSessionManager>>
{
...
mod tests {
    ...
    #[tokio::test]
    async fn registry_contains_58_tools() {
        // TODO(Phase-12): also assert the MCP tools/list surface exposes all 58
        ...
        // runtime count on it, but we can verify the registry has 64 tools.
        let registry = build_v1_registry();
        assert_eq!(
            registry.names().len(),
            64,
            "registry must have 64 tools matching the 64 #[tool] methods"
        );

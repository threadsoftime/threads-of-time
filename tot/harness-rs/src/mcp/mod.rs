//! MCP adapter — exposes the 50-tool V1 surface over the MCP StreamableHTTP
...
mod tests {
    ...
    #[tokio::test]
    async fn registry_contains_50_tools() {
        // TODO(Phase-12): also assert the MCP tools/list surface exposes all 50
        ...
        // runtime count on it, but we can verify the registry has 50 tools.
        let registry = build_v1_registry();
        assert_eq!(
            registry.names().len(),
            50,
            "registry must have 50 tools matching the 50 #[tool] methods"
        );
